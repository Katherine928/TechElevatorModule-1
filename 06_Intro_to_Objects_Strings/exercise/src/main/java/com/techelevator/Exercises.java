...
public class Exercises {
	...
	public boolean hasBad(String str) {
		if (str.equals("bad")) {
			...
			return str.startsWith("bad") || str1.startsWith("bad");
		}
		return false;
	}
	...
	public int countXX(String str) {
		...
		for (int i = 0; i < str.length() - 1; i++) {
			...
			if (letter1.equals("x") && letter2.equals("x")) {
				count += 1;
			}
		}
		return count;
	...
	public String stringSplosion(String str) {
		...
		for (int i = 1; i <= str.length(); i++) {
			str1 = str1.concat(str.substring(0,i));
		}
		return str1;
	}
	...
	public String altPairs(String str) {
		...
		if (str.length() == 0) {
			...
			for (int i = 0; i < str.length()-1; i += 4) {
				rec = rec.concat(str.substring(i, i + 2));
			}
			if (str.length() % 2 == 1 && str.length() > 4) {
				rec = rec.concat(str.substring(str.length()-1));
				return rec;
			} else {
				return rec;
			}
